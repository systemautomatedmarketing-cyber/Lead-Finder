      ...
      <div style={{ padding: "20px 16px 0" }} className="anim">
        ...
        {tab === "team" && (
          <div>
            ...
            ) : team.map(m => <MemberCard key={m.id} m={m} onUpdateLevel={updateLevel} onSelect={setSelectedMember} fire={fire} />)}
          </div>
        )}
        ...
        {tab === "rischio" && (
          <div>
            ...
            ) : atRisk.map(m => <MemberCard key={m.id} m={m} highlight="risk" onUpdateLevel={updateLevel} onSelect={setSelectedMember} fire={fire} />)}
          </div>
        )}
        ...
        {tab === "top" && (
          <div>
            ...
            ) : topPerformers.map(m => <MemberCard key={m.id} m={m} highlight="top" onUpdateLevel={updateLevel} onSelect={setSelectedMember} fire={fire} />)}
          </div>
        )}
...
function MemberCard({ m, highlight, onUpdateLevel, fire }) {
  const { T } = useTheme();
