...
const STATUS_CONFIG = {
  ...
  contattato:    { label: "Contattato",    color: "#1A5FA8", bg: "rgba(96,165,250,0.15)"  },
  interessato:   { label: "Interessato",   color: "#B8860B", bg: "rgba(184,134,11,0.15)"  },
  followup:      { label: "Follow-up",     color: "#6B4FA8", bg: "rgba(167,139,250,0.15)" },
  presentato:    { label: "Presentato",    color: "#C05A1A", bg: "rgba(251,146,60,0.15)"  },
  convertito:    { label: "✅ Cliente",    color: "#1A7A4A", bg: "rgba(62,207,142,0.15)"  },
...
export default function CollaboratoreDashboard() {
  ...
  // Form nuovo contatto
  const [nc, setNc] = useState({ name: "", type: "cliente", status: "nuovo", channel: "", note: "" });
  ...
  const addContact = async () => {
    ...
    await addDoc(collection(db, "users", uid, "contacts"), { ...nc, createdAt: serverTimestamp() });
    setNc({ name: "", type: "cliente", status: "nuovo", channel: "", note: "" });
    fire("Contatto aggiunto 📋");
  };

  const updateStatus = async (contactId, status) => {
    await updateDoc(doc(db, "users", uid, "contacts", contactId), { status, updatedAt: serverTimestamp() });
            ...
            <Card style={{ marginBottom: 14 }}>
              ...
              {weeklyClients >= weekGoal ? (
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  {currentWeek < 7 && <Btn variant="soft" onClick={advanceWeek} style={{ fontSize: 12 }}>Settimana {currentWeek + 1} →</Btn>}
                </div>
              ) : (
                <Btn onClick={registerClient} style={{ width: "100%", textAlign: "center" }}>+ Registra Cliente / Collaboratore</Btn>
              )}
            </Card>
        ...
        {tab === "contatti" && (
          <div>
            ...
            <Card style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>+ Nuovo Contatto</div>
            ...
            {contacts.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0", color: T.muted, fontFamily: "'DM Sans'" }}>
                ...
                <div style={{ lineHeight: 1.8 }}>Inizia con la Lista dei 20!<br />Aggiungi i tuoi primi contatti qui.</div>
              </div>
            ) : contacts.map(c => {
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, fontFamily: "'DM Sans'", color: T.muted, marginTop: 3 }}>
                        {c.type === "cliente" ? "👤" : "🤝"} {c.channel && `· ${c.channel}`}
                      </div>
