...
// Dashboard leader compatta per collaboratori con doppio ruolo.
// NON chiama useDualRole() internamente — riceve tutto via props
// dal CollaboratoreDashboard che è l'unico owner dell'hook.
...
// ── Props ricevute dal parent (CollaboratoreDashboard):
//   onSwitchBack    — torna alla vista collaboratore
//   teamMembers     — array collaboratori del team
//   myInviteCode    — codice invito del sotto-leader
//   uplineName      — nome del proprio upline
//   promoteToLeader — funzione per promuovere un membro
//   promoting       — boolean loading stato promozione
//   generateMyInviteCode — genera e salva il codice invito
export default function MiniLeaderDashboard({
  onSwitchBack,
  teamMembers = [],
  myInviteCode,
  uplineName,
  ...
  const handlePromote = async (member) => {
    if (member.isLeader || !promoteToLeader) return;
  ...
  const card = {
    ...
    borderRadius: 14,
    padding: 16,
    boxShadow: T.shadowCard || "0 1px 4px rgba(0,0,0,0.06)",
  };

  // Colori sicuri con fallback
  const safeGreen  = T.green  || "#1A7A4A";
  const safeRed    = T.red    || "#C0392B";
  const safeRedBg  = T.redBg  || "rgba(192,57,43,0.08)";
  const safeAccentSoft = T.accentSoft || T.accent;
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes mldFadeIn { from { opacity:0; transform:translateY(6px); } to { opacity:1; transform:translateY(0); } }
        .mld-anim { animation: mldFadeIn 0.25s ease; }
        .mld-select {
        ...
        .mld-select option { background: ${T.surface}; }
      `}</style>
      ...
      <div style={{ padding: "24px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            onClick={logout}
            style={{ background: "none", border: `1px solid ${T.border}`, color: T.muted, padding: "6px 10px", borderRadius: 50, fontSize: 12, fontFamily: "'DM Sans'", cursor: "pointer" }}
          >
            ↩
          </button>
        </div>
      ...
      <div style={{ padding: "16px 16px 0" }} className="mld-anim">

        {/* Torna alla vista collaboratore */}
        <button
          onClick={onSwitchBack}
          style={{ display: "flex", alignItems: "center", gap: 8, background: T.accentBg, border: `1px solid ${T.accentBorder}`, borderRadius: 50, padding: "8px 16px", fontFamily: "'DM Sans'", fontWeight: 700, fontSize: 12, color: T.accent, cursor: "pointer", marginBottom: 16 }}
        >
        ...
        <div style={{ background: T.accentBg, border: `1px solid ${T.accentBorder}`, borderRadius: 14, padding: "14px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 11, color: T.accent, fontFamily: "'DM Sans'", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>
              Il tuo codice invito
            </div>
            {myInviteCode ? (
              <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: 3, color: T.accent }}>
                {myInviteCode}
              </div>
            ) : (
              <div style={{ fontSize: 13, fontFamily: "'DM Sans'", color: T.muted }}>
                Genera il codice per iniziare a reclutare
              </div>
            )}
          </div>
          <button
            ...
            style={{ background: T.accent, border: "none", color: "#0a0a0f", padding: "9px 16px", borderRadius: 50, fontFamily: "'DM Sans'", fontWeight: 700, fontSize: 12, cursor: "pointer", flexShrink: 0 }}
          >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 20 }}>
          {[
            ...
            { label: "Top",     value: topPerformers.length, icon: "⭐", color: safeGreen },
            { label: "Rischio", value: atRisk.length,        icon: "⚠️", color: atRisk.length > 0 ? safeRed : T.muted },
          ].map(k => (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            ...
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              style={{ background: tab === t.id ? T.accent : T.surface, border: `1px solid ${tab === t.id ? T.accent : T.border}`, color: tab === t.id ? "#0a0a0f" : T.muted, padding: "7px 14px", borderRadius: 50, fontSize: 11, fontFamily: "'DM Sans'", fontWeight: 700, cursor: "pointer" }}
            >
              {t.label}
            </button>
          ))}
            ...
            {teamMembers.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0", color: T.muted, fontFamily: "'DM Sans'" }}>
                ...
                <div style={{ lineHeight: 1.8 }}>
                  ...
                  {myInviteCode
                    ? <span>Condividi il codice <strong style={{ color: T.accent }}>{myInviteCode}</strong></span>
        ...
        {tab === "rischio" && (
          <div>
            {atRisk.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0", color: safeGreen, fontFamily: "'DM Sans'" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
        ...
        {tab === "azioni" && (
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 14, color: T.text }}>
              💡 Azioni Leader questa settimana
            </div>
            {[
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      ...
                      <span style={{
                        background: a.urgency === "alta" ? safeRedBg : T.accentBg,
                        color: a.urgency === "alta" ? safeRed : T.accent,
                        padding: "2px 8px", borderRadius: 20, fontSize: 10, fontFamily: "'DM Sans'", fontWeight: 700,
                      }}>
      ...
      {/* Bottom nav */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: T.navBg || T.surface, borderTop: `1px solid ${T.border}`, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", backdropFilter: "blur(12px)" }}>
        <div style={{ fontFamily: "'DM Sans'", fontSize: 12, color: T.muted }}>
          👑 Leader · {teamMembers.length} collaboratori
        </div>
        <button
          onClick={onSwitchBack}
          style={{ background: T.accentBg, border: `1px solid ${T.accentBorder}`, color: T.accent, padding: "7px 16px", borderRadius: 50, fontFamily: "'DM Sans'", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
        >
          ⚡ Vista Collaboratore
        </button>
      </div>
...
function MemberCard({ m, T, card, highlight, safeRed, safeRedBg, safeAccentSoft, onPromote, onUpdateLevel, promoting, fire }) {
  ...
  return (
    <div style={{ ...card, marginBottom: 12, border: `1px solid ${isRisk ? safeRed + "44" : m.isLeader ? T.accent + "44" : T.border}` }}>
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            {m.isLeader && (
              <span style={{ background: T.accentBg, color: T.accent, padding: "2px 8px", borderRadius: 20, fontSize: 10, fontFamily: "'DM Sans'", fontWeight: 700 }}>
                👑 Leader
              </span>
            )}
          </div>
      ...
      <div style={{ background: T.border, borderRadius: 50, height: 5, marginBottom: 12 }}>
        <div style={{ height: 5, borderRadius: 50, background: isRisk ? safeRed : `linear-gradient(90deg, ${T.accent}, ${safeAccentSoft})`, width: `${pct}%`, transition: "width 0.5s" }} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {isRisk && (
          <button
            onClick={() => {
              navigator.clipboard?.writeText(`Ciao ${m.name}! Come va? Sono qui per supportarti questa settimana 💪`);
              fire("Messaggio copiato!");
            }}
            style={{ background: safeRedBg, border: `1px solid ${safeRed}44`, color: safeRed, padding: "5px 12px", borderRadius: 50, fontSize: 11, fontFamily: "'DM Sans'", fontWeight: 700, cursor: "pointer" }}
          >
            ✉️ Supporto
          </button>
        )}
